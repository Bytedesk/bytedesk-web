...
var httpapi = {
  ...
  requestUsername: function () {
    //
    data.username = localStorage.bd_kfe_username;
    data.password = localStorage.bd_kfe_password;
    if (data.username) {
      ...
      $.ajax({
        ...
        success:function(response){
          ...
          // 本地存储
          localStorage.bd_kfe_uid = data.uid;
          localStorage.bd_kfe_username = data.username;
          // localStorage.bd_kfe_password = data.password;
          // 登录
          httpapi.login();
        },
  ...
  login: function () {
    ...
    $.ajax({
      ...
      success:function(response){
        ...
        localStorage.bd_kfe_username = data.username;
        localStorage.bd_kfe_password = data.password;
        // localStorage.bd_kfe_subDomain = data.subDomain;
        // localStorage 存储
  ...
  browse: function () {
    ...
    $.ajax({
      ...
      beforeSend: function (xhr) {
        xhr.setRequestHeader('Authorization', 'Bearer ' + data.passport.token.access_token);
      },
      success:function(response){
    ...
    $.ajax({
      ...
      success:function(response){
        ...
        if (response.status_code === 200) {
          ...
          // 前置选择
          data.questionnaireItemItems = message.questionnaire.questionnaireItems[0].questionnaireItemItems;
          // 插入业务路由，相当于咨询前提问问卷（选择 或 填写表单）
          bd_kfe_utils.pushToMessageArray(message);
          // 1. 保存thread
          data.thread = message.thread;
    ...
    $.ajax({
      ...
      success:function(response){
        ...
        if (response.status_code === 200) {
          ...
          bd_kfe_utils.pushAnswers(data.answers.content);
        } else {
          alert(response.message);
        }
      },
