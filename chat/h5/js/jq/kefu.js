...
var kefu = {
  ...
  created: function() {
    // 单独窗口形式
    data.adminUid = bd_kfe_utils.getUrlParam("uid");
    data.workGroupWid = bd_kfe_utils.getUrlParam("wid");
    data.subDomain = bd_kfe_utils.getUrlParam("sub");
    data.type = bd_kfe_utils.getUrlParam("type");
    data.agentUid = bd_kfe_utils.getUrlParam("aid");
    //
    data.uid = localStorage.bd_kfe_uid;
  ...
  mounted: function() {
    ...
    if (
      ...
      if (data.username === bd_kfe_utils.getUrlParam("username")) {
        // 非第一次使用自定义用户名
        httpapi.login();
      } else {
        // 保存自定义用户名到服务器
    ...
    } else if (bd_kfe_utils.getUrlParam("username")) {
      // 保存自定义用户名到服务器
...
$(document).ready(function () {
 ...
 document.addEventListener('visibilitychange', function(){
   if (document.visibilityState === 'hidden') {
     data.browserTabHidden = true
   } else if (document.visibilityState === 'visible') {
     data.browserTabHidden = false;
