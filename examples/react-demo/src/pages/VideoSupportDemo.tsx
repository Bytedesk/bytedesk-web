...
const VideoSupportDemo = ({ locale, themeMode, selectedChatProfile, selectedUser, isAnonymousMode, onSelectUser, onAnonymousModeChange }: DemoPageProps) => {
  ...
  const config = useMemo<BytedeskConfig>(() => ({
    ...
    bubbleConfig: {
      show: true,
      icon: '🎥',
      title: messages.pages.videoSupportDemo.bubbleTitle,
      subtitle: messages.pages.videoSupportDemo.bubbleSubtitle
    },
    buttonConfig: {
      show: true,
      width: 60,
  ...
  const docLinks = useMemo(
    () => [
      { href: 'https://www.weiyuai.cn/docs/zh-CN/docs/channel/react', label: messages.pages.videoSupportDemo.docLinks.reactDoc },
      { href: 'https://www.weiyuai.cn/docs/zh-CN/docs/channel/vue', label: messages.pages.videoSupportDemo.docLinks.vueDoc },
      { href: 'https://github.com/Bytedesk/bytedesk-web/blob/master/examples/react-demo/src/pages/VideoSupportDemo.tsx', label: messages.pages.videoSupportDemo.docLinks.reactExample }
    ],
    [messages]
  );
      ...
      <Card>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          <Typography.Title level={2} style={{ marginBottom: 0 }}>
            {messages.pages.videoSupportDemo.title}
          </Typography.Title>
          <Typography.Paragraph type="secondary" style={{ marginBottom: 0 }}>
            {messages.pages.videoSupportDemo.description}
          ...
          <Alert
            type="info"
            showIcon
            message={messages.pages.videoSupportDemo.pathAlert}
          />
        </Space>
      </Card>
      ...
      <Card>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          <Space align="center" wrap>
            <Typography.Text strong>{messages.pages.videoSupportDemo.currentPathLabel}</Typography.Text>
            <Tag color="processing">/chat</Tag>
            <Typography.Text type="secondary">{messages.pages.videoSupportDemo.currentPathHint}</Typography.Text>
          </Space>
          ...
          <Space direction="vertical" size={8} style={{ width: '100%' }}>
            <CurrentUserProfile
              ...
              avatar={selectedUser.avatar}
              anonymousIdText={messages.pages.videoSupportDemo.anonymousUserHint}
            ...
            <Space wrap>
              ...
              >
                {messages.pages.videoSupportDemo.buttons.switchAnonymousUser}
                {isAnonymousMode ? '【当前】' : ''}
              </Button>
            </Space>
          </Space>
          ...
          <List
            header={messages.pages.videoSupportDemo.usageTitle}
            dataSource={messages.pages.videoSupportDemo.usageNotes}
            renderItem={(note) => <List.Item>{note}</List.Item>}
          />
        </Space>
      </Card>

      <Card title={messages.pages.videoSupportDemo.urlGuideTitle}>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          <Typography.Text strong>{messages.pages.videoSupportDemo.sampleUrlLabel}</Typography.Text>
...
export default VideoSupportDemo;
