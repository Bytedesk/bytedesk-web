...
const VideoConferenceDemo = ({ locale, themeMode, selectedChatProfile, selectedUser, isAnonymousMode, onSelectUser, onAnonymousModeChange }: DemoPageProps) => {
  ...
  const config = useMemo<BytedeskConfig>(() => ({
    ...
    bubbleConfig: {
      show: true,
      icon: '🎬',
      title: messages.pages.videoConferenceDemo.bubbleTitle,
      subtitle: messages.pages.videoConferenceDemo.bubbleSubtitle
    },
    buttonConfig: {
      show: true,
      width: 60,
  ...
  const docLinks = useMemo(
    () => [
      { href: 'https://www.weiyuai.cn/docs/zh-CN/docs/channel/react', label: messages.pages.videoConferenceDemo.docLinks.reactDoc },
      { href: 'https://www.weiyuai.cn/docs/zh-CN/docs/channel/vue', label: messages.pages.videoConferenceDemo.docLinks.vueDoc },
      { href: 'https://github.com/Bytedesk/bytedesk-web/blob/master/examples/react-demo/src/pages/VideoConferenceDemo.tsx', label: messages.pages.videoConferenceDemo.docLinks.reactExample }
    ],
    [messages]
  );
      ...
      <Card>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          <Typography.Title level={2} style={{ marginBottom: 0 }}>
            {messages.pages.videoConferenceDemo.title}
          </Typography.Title>
          <Typography.Paragraph type="secondary" style={{ marginBottom: 0 }}>
            {messages.pages.videoConferenceDemo.description}
          ...
          <Alert
            type="info"
            showIcon
            message={messages.pages.videoConferenceDemo.pathAlert}
          />
        </Space>
      </Card>
      ...
      <Card>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          <Space align="center" wrap>
            <Typography.Text strong>{messages.pages.videoConferenceDemo.currentPathLabel}</Typography.Text>
            <Tag color="processing">/chat</Tag>
            <Typography.Text type="secondary">{messages.pages.videoConferenceDemo.currentPathHint}</Typography.Text>
          </Space>
          ...
          <Space direction="vertical" size={8} style={{ width: '100%' }}>
            <CurrentUserProfile
              ...
              avatar={selectedUser.avatar}
              anonymousIdText={messages.pages.videoConferenceDemo.anonymousUserHint}
            ...
            <Space wrap>
              ...
              >
                {messages.pages.videoConferenceDemo.buttons.switchAnonymousUser}
                {isAnonymousMode ? '【当前】' : ''}
              </Button>
            </Space>
          </Space>
          ...
          <List
            header={messages.pages.videoConferenceDemo.usageTitle}
            dataSource={messages.pages.videoConferenceDemo.usageNotes}
            renderItem={(note) => <List.Item>{note}</List.Item>}
          />
        </Space>
      </Card>

      <Card title={messages.pages.videoConferenceDemo.urlGuideTitle}>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          <Typography.Text strong>{messages.pages.videoConferenceDemo.sampleUrlLabel}</Typography.Text>
